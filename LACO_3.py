...
class BERTClass(torch.nn.Module):
    ...
    def forward(self, ids, mask, token_type_ids, label_positions, doc_positions):
        ...
        token_label_output_layer = torch.reshape(token_label_output_layer, [-1, self.num_classes, token_label_hidden_size])
...
def gather_indexes(sequence_tensor, positions, args):
    ...
    flat_positions = flat_positions.unsqueeze(1).expand(1,flat_positions.shape[0], flat_sequence_tensor.shape[1]).squeeze()
...
def bce_loss_fn(outputs, targets):
    """定义BCE损失函数"""
...
def model_train(args, train_df):
    ...
    for epoch in range(args.epochs):
        ...
        for _, data in enumerate(tqdm(train_loader), 0):
            ...
            loss = bce_loss_fn(outputs, targets)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', default='roberta_LACO', type=str)
    ...
    train_data = []
    test_data = []
    if not os.path.exists(args.data_path + 'LACO_train_data_6_01.npy'):
        ...
        print('保存训练数据...')
        np.save(args.data_path + 'LACO_train_data_6_01.npy', train_data)
    else:
        print('载入训练数据...')
        train_data = np.load(args.data_path + 'LACO_train_data_6_01.npy', allow_pickle=True)
    if not os.path.exists(args.data_path + 'LACO_test_data_6_01.npy'):
        for i in range(len(test)):
            print(i)
            test_da = tokenizer.convert_inputs(test['text'].to_list()[i], args.max_len, is_dev = True)
            test_data.append(test_da)
        print('保存训练数据...')
        np.save(args.data_path + 'LACO_test_data_6_01.npy', test_data)
    else:
        print('载入测试数据...')
        test_data = np.load(args.data_path + 'LACO_test_data_6_01.npy', allow_pickle=True)
    ...
    LACO_train_data_raw.npy:标签6个，0-3
    LACO_train_data_6_01.npy：标签6个，0-1
    LACO_train_data_24.npy:标签24个
    
    0.6839roberta_LACO.tsv：早停，6标签0-1，分层权重衰减，梯度裁剪_5，学习率的调整warmup
    0.6845roberta_LACO.tsv：早停，6标签0-1，梯度裁剪_5
    0.6843：早停，6标签0-1
