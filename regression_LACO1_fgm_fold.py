...
class joint_embedding(torch.nn.Module):
    ...
    def __init__(self, args):
        ...
        self.ngram_size = [2, 3, 4]
        self.convs_doc = nn.ModuleList([nn.Conv1d(104, 104, K) for K in self.ngram_size])
        self.relu = nn.ReLU()
    ...
    def forward(self, doc_output_layer, token_label_output_layer):
        ...
        G = torch.matmul(x_emb_norm, w_class_norm)  # [32,104,6]
        ...
        for i in range(len(self.ngram_size)):
            nets = self.convs_doc[i](G)
            nets = self.relu(nets)
            nets_doc_l.append(nets)
        Att_v = torch.cat((nets_doc_l[0], nets_doc_l[1], nets_doc_l[2]), 2)
        ...
        Att_v = torch.max(Att_v, dim=-1, keepdims=True)[0]  # [32,104,1]前一个[32,104,6]
...
def get_new_labels(y):
    """转换标签"""
    y_new = LabelEncoder().fit_transform([''.join(str(l['target_label'])) for l in y])
    return y_new
...
def model_train(args, train_df):
    ...
    train_df_new_label = get_new_labels(train_df)
    skf = StratifiedKFold(n_splits=args.nfolds, shuffle=True, random_state=args.seed_value)

    for i, (trn_idx, val_idx) in enumerate(skf.split(train_df, train_df_new_label)):
        print('--------------------------------- {} fold ---------------------------------'.format(i + 1))
        trn_df = train_df[trn_idx]
        dev_df = train_df[val_idx]

        train_dataset = BERTDataset(trn_df, args.max_len)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', default='regression_LACO1_fgm_fold', type=str)
    ...
    train_data = []
    test_data = []
    if not os.path.exists(args.data_path + 'LACO_train_data_raw_persona_text.npy'):
        ...
        print('保存训练数据...')
        np.save(args.data_path + 'LACO_train_data_raw_persona_text.npy', train_data)
    else:
        print('载入训练数据...')
        train_data = np.load(args.data_path + 'LACO_train_data_raw_persona_text.npy', allow_pickle=True)
    if not os.path.exists(args.data_path + 'LACO_test_data_raw_persona_text.npy'):
        for i in range(len(test)):
            print(i)
            test_da = tokenizer.convert_inputs(test['text'].to_list()[i], args.max_len, is_dev=True)
            test_data.append(test_da)
        print('保存训练数据...')
        np.save(args.data_path + 'LACO_test_data_raw_persona_text.npy', test_data)
    else:
        print('载入测试数据...')
        test_data = np.load(args.data_path + 'LACO_test_data_raw_persona_text.npy', allow_pickle=True)
