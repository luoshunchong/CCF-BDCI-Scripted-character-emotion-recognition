...
class convert_input(object):
    ...
    def convert_inputs(self, text, max_seq_length, target=None, is_dev=False):
        ...
        if len(fit_labelspace_given_positions) == 0:
            fit_labelspace_given_positions.append('#')
        else:
            fit_labelspace_mask_positions.append('#')
            mask_lm_ids.append('#')
...
class BERTDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        fit_labelspace_mask_positions = text['fit_labelspace_mask_positions']
        fit_labelspace_given_positions = text['fit_labelspace_given_positions']
        mask_lm_ids = text['mask_lm_ids']
        ...
        if self.is_dev:
            return {
                ...
                'fit_labelspace_mask_positions': torch.LongTensor(fit_labelspace_mask_positions),
                'fit_labelspace_given_positions': torch.LongTensor(fit_labelspace_given_positions),
                'mask_lm_ids': torch.LongTensor(mask_lm_ids)
            }
        else:
            return {
                'ids': torch.LongTensor(ids),
...
class BERTClass(torch.nn.Module):
    ...
    def __init__(self, args):
        ...
        self.input_tensor_mask_fc = nn.Linear(768, 768)
        self.input_tensor_mask_fc.weight = truncated_normal_(token_label_output_weight, std=0.02)
    ...
    def forward(self, ids, mask, token_type_ids, label_positions, doc_positions, fit_labelspace_mask_positions, fit_labelspace_given_positions, mask_lm_ids):
        ...
        input_tensor_mask = self.input_tensor_mask_fc(input_tensor_mask)
        input_tensor_mask = self.gelu(input_tensor_mask)
...
def model_train(args, train_df):
    ...
    for epoch in range(args.epochs):
        ...
        for _, data in enumerate(tqdm(train_loader), 0):
            ...
            fit_labelspace_mask_positions = data['fit_labelspace_mask_positions'].to(args.device)
            fit_labelspace_given_positions = data['fit_labelspace_given_positions'].to(args.device)
            mask_lm_ids = data['mask_lm_ids'].to(args.device)
            # # 平滑标签
            # targets = (1 - args.epsilon) * targets + args.epsilon / args.num_classes
            optimizer.zero_grad()
            outputs = model(ids, mask, token_type_ids, label_positions, doc_positions, fit_labelspace_mask_positions, fit_labelspace_given_positions, mask_lm_ids)
        ...
        RMSE = metrics.mean_squared_error(fin_targets, fin_outputs, squared=False)
...
def model_predict(args, test, submit):
    ...
    for step, batch in enumerate(tqdm(test_loader), 0):
        ...
        fit_labelspace_mask_positions = batch['fit_labelspace_mask_positions'].to(args.device)
        fit_labelspace_given_positions = batch['fit_labelspace_given_positions'].to(args.device)
        mask_lm_ids = batch['mask_lm_ids'].to(args.device)
        with torch.no_grad():
            outputs = test_model(ids, mask, token_type_ids, label_positions, doc_positions, fit_labelspace_mask_positions, fit_labelspace_given_positions, mask_lm_ids)
    ...
    pred_end = [[_ for _ in example] for example in preds]
    sub = submit.copy()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', default='regression_LACO', type=str)
    ...
    train_data = []
    test_data = []
    if not os.path.exists(args.data_path + 'LACO_clcp_train_data_raw_persona_text.npy'):
        ...
        print('保存训练数据...')
        np.save(args.data_path + 'LACO_clcp_train_data_raw_persona_text.npy', train_data)
    else:
        print('载入训练数据...')
        train_data = np.load(args.data_path + 'LACO_clcp_train_data_raw_persona_text.npy', allow_pickle=True)
    if not os.path.exists(args.data_path + 'LACO_clcp_test_data_raw_persona_text.npy'):
        for i in range(len(test)):
            print(i)
            test_da = tokenizer.convert_inputs(test['text'].to_list()[i], args.max_len, is_dev=True)
            test_data.append(test_da)
        print('保存训练数据...')
        np.save(args.data_path + 'LACO_clcp_test_data_raw_persona_text.npy', test_data)
    else:
        print('载入测试数据...')
        test_data = np.load(args.data_path + 'LACO_clcp_test_data_raw_persona_text.npy', allow_pickle=True)
