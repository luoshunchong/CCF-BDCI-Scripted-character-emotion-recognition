...
from transformers import BertTokenizer, AutoTokenizer, BertModel, AutoModel, AutoModelForMaskedLM, AutoModelWithLMHead, \
...
class BERTClass(torch.nn.Module):
    ...
    def __init__(self, args):
        ...
        self.bert = NeZhaModel.from_pretrained(args.roberta_path)
        # self.bert = BertModel.from_pretrained(args.roberta_path)
        #         self.bert = AutoModelForMaskedLM.from_pretrained(args.roberta_path)
    ...
    #                               hidden_size=768, batch_first=True, bidirectional=True)

    def forward(self, ids, mask, token_type_ids):
        a = self.bert(input_ids=ids, token_type_ids=token_type_ids, attention_mask=mask)
        sequence_output = a[0]
        pooler_output = a[1]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', default='regression_nezha_fgm', type=str)
    parser.add_argument('--data_path', default='./raw_data/', type=str)
    parser.add_argument('--roberta_path', default='./pre_models/nezha-chinese-base/', type=str)
    #     parser.add_argument('--roberta_path', default='peterchou/nezha-chinese-base', type=str) # 运行时自动下载
