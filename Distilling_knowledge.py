...
def model_train(args, train_df, tokenizer):
    ...
    for epoch in range(args.epochs):
        ...
        for _, data in enumerate(tqdm(train_loader), 0):
            ...
            # KD
            with torch.no_grad():
                kd_outputs = kd_model(ids, mask, token_type_ids)
            kd_loss = mse_loss_fn(outputs, kd_outputs)
            loss += kd_loss
...
"""
采用知识蒸馏技术：
方案一：teacher model和students model都同时采用regression_roberta_fgm
方案二：teacher model采用regression_LACO1_fgm，students model采用regression_roberta_fgm
"""