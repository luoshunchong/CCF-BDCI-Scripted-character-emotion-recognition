...
class joint_embedding(torch.nn.Module):
    ...
    def __init__(self, args):
        ...
        self.convs_doc = nn.ModuleList([nn.Conv1d(114, 114, K) for K in self.ngram_size])
        self.relu = nn.ReLU()
...
def model_train(args, train_df):
    ...
    trn_df, dev_df = train_test_split(train_df, test_size=0.2, random_state=args.seed_value)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', default='regression_LACO1_fgm', type=str)
    ...
    train_data = []
    test_data = []
    if not os.path.exists(args.data_path + 'LACO_train_data_3j.npy'):
        ...
        print('保存训练数据...')
        np.save(args.data_path + 'LACO_train_data_3j.npy', train_data)
    else:
        print('载入训练数据...')
        train_data = np.load(args.data_path + 'LACO_train_data_3j.npy', allow_pickle=True)
    if not os.path.exists(args.data_path + 'LACO_test_data_3j.npy'):
        for i in range(len(test)):
            print(i)
            test_da = tokenizer.convert_inputs(test['text'].to_list()[i], args.max_len, is_dev=True)
            test_data.append(test_da)
        print('保存训练数据...')
        np.save(args.data_path + 'LACO_test_data_3j.npy', test_data)
    else:
        print('载入测试数据...')
        test_data = np.load(args.data_path + 'LACO_test_data_3j.npy', allow_pickle=True)
